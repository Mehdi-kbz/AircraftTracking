...
for j = 1:length(Eb_N0_dB)
    while compteur_erreurs < 2000
            ...
            phi_0 = 0;                              % Déphasage 0 pour la tache 5
            ...
            %%% Calculer le signal reçu (désynchronisé fréquentiellement) %%%
            yl = alpha * sl_p .* exp(-1j * 2 * pi * delta_f * (0:length(sl_p)-1) * Ts) .* exp(1j * phi_0)   ;
...
%{
            ...
            %%%%%%%%%%%%%%%%%%% Synchronisation temporelle du signal %%%%%%%%%%%%%%%%%
            yl_synchronise_temporellement = yl(delta_samples_estime  + 1 : end);
            
%}

            %%%%%%%%%%%%%%%%%%%% Estimation de delta_f %%%%%%%%%%%%%%%%%%%%
            %delta_f_estime = Estimation_delta_f(yl, s_p, 1/Fe);
            delta_f_estime = Estimer_delta_f(Fse,s_p,yl,M,Te);
            %%%%%%%%%%%%%%%%%%% Synchronisation fréquentielle du signal %%%%%%%%%%%%%%%%%
            yl_synchronise_frequentiellement = yl .* exp(1j * 2 * pi * delta_f_estime * (0:length(sl_p)-1) * Ts);
            ...
            rl = conv(yl_synchronise_frequentiellement, filtre_adapte, 'same'); 
...
fprintf('Décalage fréquentiel réel: %.6f Hz\n', delta_f * 1e6);
fprintf('Décalage fréquentiel estimé: %.6f Hz\n', delta_f_estime * 1e6);
fprintf("Erreur d'éstimation: %.6f Hz\n", abs(delta_f - delta_f_estime) * 1e6);
...
figure;
%%% Tracé du signal désynchronisé fréquentiellement %%%
subplot(2, 1, 1); 
plot(abs(yl));
title('Signal désynchronisé');
legend('yl');

%%% Tracé du signal synchronisé fréquentiellement %%%
subplot(2, 1, 2); 
plot(abs(yl_synchronise_frequentiellement));
